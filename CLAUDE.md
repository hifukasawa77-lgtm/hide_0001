...
- **静的検査の解決先は「そのファイル自身が宣言している `assets/.../` 」を使う**。
  `assets` 直下1階層だけを候補にしていたため、2階層のルートを持つゲーム
  （`HEX_BATTLE_ROOT = 'assets/genpei/hex-battles/'`）の参照29件が丸ごと
  「実在しない」と**偽の赤**になっていた（2026-09-06 修正）。
  候補を推測で広げると今度は誤った場所で解決して**本物の間違いを見逃す**ので、広げない
- **別オリジン（CDN・Webフォント）の読込失敗はFAILにしない**（`△ 外部` として表示のみ）。
  回線・地域・社内プロキシで落ちるものを混ぜると、本物の404が環境由来の赤に埋もれる。
  ローカル資産の404・ページ例外は従来どおりFAIL（`dynamic-test-auto.cjs` と同じ設計）
- **検査自身のクリック・Enterで始まった遷移の中断（`ERR_ABORTED`）は、実体が在るときだけ除外する**。
  `card-games.html` → 実在する `poker.html` が「404」と報告されていた。
  実体が無いファイルの中断は従来どおりFAIL（緩めると本物の欠落を見逃す）
...
- **例外はZERO-1の推論だけ**。画像生成・音声・ストック素材等に広げない
- **ポートフォリオ側の「クラウド頭脳（BYOK）」とは別の話**。`zero-1-mobile.html` は
  **訪問者が自分の鍵をブラウザへ入れる**方式で、費用を払うのはその訪問者。深澤には
  課金されないので、この¥3,000枠とは関係しない。**リポジトリにも深澤のPCにも鍵を置かない**
  （置いた瞬間、訪問者の利用が深澤の課金になる）
