...
### 唯一の例外: ZERO-1 のクラウドLLM（2026-09-06 深澤承認・月¥3,000）

**別リポジトリ `zero-1-local-ai`（PC版 ZERO-1）に限り**、クラウドLLMの利用を認める。
金額の正本は `accounting/budget.md` の `ZERO1_API_LIMIT`（全体 `MONTHLY_LIMIT` の**内数**）。

- **例外はZERO-1の推論だけ**。このリポジトリ（ポートフォリオ）側では従来どおり課金ゼロ。
  画像生成・音声・ストック素材等に広げない
- **鍵はZERO-1のサーバ側 `.env.local` にのみ置く**（`.env*` は gitignore 済み）。
  クライアントコード・このリポジトリには一切書かない。
  **この例外があっても「鍵をコミットしない」は変わらない**——誤ってコミットしたら即 revoke
- **上限は仕組みで守る**。①提供元の管理画面のハード上限（人が設定する）②ZERO-1 が呼ぶ前に
  当月消費を見て止める（`app/api-budget.ts`）。「気をつける」では守れない
- **単価が分からないモデルは呼ばない**。価格表に無いモデルは止める（推測で見積もると上限が
  上限でなくなる）。**モデル名と単価は公式の料金ページで確認してから登録する**——
  伝聞のモデル名は実在しないことがある（2026-09-06、実在を確認できない名前と単価で
  見積もりが持ち込まれた）
