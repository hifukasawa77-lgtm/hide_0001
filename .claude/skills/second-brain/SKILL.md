...
obsidian-vault/
  ...
  00-Inbox/       # 未整理の一時メモ・思いつき（スマホのObsidianからの投げ込み先）
...
### 📥 セッション開始時: 00-Inbox/ の受信メモを必ず処理する
recall hook が `00-Inbox/` の全メモを**全文**コンテキストへ入れる。これは深澤がスマホの Obsidian から
投げ込んだ未整理メモで、**読んで振り分けるところまでやって初めて「記憶した」ことになる**。

1. 内容を読み、行き先を決める:
   意思決定→`03-Decisions/`（ADR形式） / 再利用できる学び→`04-Knowledge/` /
   プロジェクトの状態→`02-Projects/` / その日の作業記録→`01-Daily/`
2. 行き先のノートへ**深澤の言葉を要約せず要点を保って**転記し、frontmatter を整える
3. 転記できたら `00-Inbox/` の元ファイルを**消す**（`はじめに.md` は消さない）。
   残すと毎セッション読み直して文脈を食い、同じメモを何度も処理する
4. 行き先が決まらないもの・意図が読み取れないものは**消さずに残し**、深澤へ
   「これはどこへ入れますか」と聞く。**推測で振り分けない**（間違った場所に入ると二度と見つからない）
5. 機微情報（鍵・パスワード・他人の個人情報）が混じっていたら、転記せず**即座に深澤へ報告**する。
   このリポジトリは公開で、スマホから書くと無自覚に混入しやすい
