...
- `assets/js/zero1-tools.js` — ZERO-1 Mobile の端末内ツール層（時計・タイマー・メモ・単位換算・計算・サイト内検索）と、サイト知識の取り出し（ローカルRAG）。純粋ロジックのみで作用を持たない
...
```bash
node scripts/verify-zero1-mobile.mjs      # 端末判定→モデル選び→起動失敗の手掛かり→会話→体裁→速さ→PWA→GPU切断→端末内ツール→サイト知識（154項目）
node scripts/verify-gesture-pointer.mjs   # エアタッチ: 手ぶれ取り→押下判定→タップ/スワイプ/ドラッグ→カメラ入切→調整・静止クリック・右クリック（84項目）
...
**端末内ツールとサイトの知識は、モデルの手前に置く**（2026-09-05 追加。`assets/js/zero1-tools.js`）。

- **モデルに `tool_calls` を書かせない**。0.5〜3B級は指示を守り切れず、`local-agent/README.md` に
  実測がある通り tools capability を持つモデルでも `{"name":…}` という**JSON文字列を本文に書く**。
  そうなるとツールは一度も発火せず、例外もエラーも出ないまま「変な文字列を喋るAI」になる。
  判定は**決定的**（正規表現＋語の重み）にして、モデルより先に確定させる。確実に答えられるものは
  GPUを一度も回さない＝即答・電池を食わない・言い間違えない
- **曖昧なら必ずモデルへ流す**。誤爆は利用者からは「AIが答えてくれなくなった」に見える。
  拾うのは「これ以外の意味が無い」形だけ。単位換算は**行き先の単位が文中に無ければ拾わない**
  （拾うと「100メートル走のコツ」に "100m = 328フィート" が返る）
- **式の評価に eval / new Function を使わない**。自前のトークナイザ＋操車場アルゴリズムで解く。
  利用者が打った文字列をそのまま評価する道を、ページ内に1本も作らない（検査#120が静的に見る）
- **タイマーは終わる時刻（絶対時刻）を正本にする**。スマホでは画面を消す・タブを背面に回すだけで
  `setTimeout` が遅れる・止まる（例外もエラーも出ない。ただ鳴らない）。`visibilitychange` で
  戻るたびに貼り直す。検査#125は「画面が消えていた間に期限が過ぎた」状態を作って鳴るかを見る
- **サイトの知識（`assets/js/agent-data.js`）は、当たったときだけ足す**。29KB＝1.5万トークン級で、
  丸ごと入れると窓が埋まり**いま聞かれた質問が押し出されて見当違いの答えが返る**（`buildMessages`
  の注意書きと同じ事故）。当たった数件だけ・`budget` で頭打ちにする。無関係なら**0文字**
- **`system` は1つに畳んで渡す**。会話の途中に2つ目の system を挟むと、チャットテンプレート
  （Qwen/Gemma系）は先頭の1つしか描かないことがあり、**例外もエラーも出ないまま知識だけが届かない**
- **KBの `{GAME_COUNT}` を、そのままモデルへ渡さない**。渡すとモデルは答えにもその記号を書き写す
  （`agent.js` の差し込みと対。検査#136）
- **モデルが答えたのか、ツールが答えたのかを画面で見分けられるようにする**。区別が付かないと
  「AIが時計を読めた」と誤解され、できないことまで期待される
- **正本は案内エージェントと同じ `agent-data.js`**。別に持つとゲームを1本足したときに片方だけ古くなり、
  ZERO-1 だけが存在しないゲームを勧めるようになる（検査#128が href の実在を見る）
...
**押せるかどうかは「閾値が合っているか」で決まる**（2026-09-06 ブラッシュアップ）。

- **つまみ方には個人差がある**。既定の閾値（0.42/0.58）は手の大きさで正規化してもなお合わない人がいて、
  合わないと**押しても反応しない**としか見えない。`PinchCalibration` で「開く→つまむ」を2.5秒ずつ実測し、
  2つの山の間へ線を引く（`deriveThresholds`）。**山が重なっていたら閾値を作らずに理由を返す**——
  無理に作ると「調整したのに前より当たらない」という最悪の結果になる
- **調整中は押下を実DOMへ通さない**。閾値を測っている最中のつまみでボタンが押されると、
  調整のたびに画面が勝手に動く。検査は**クリックではなく押し始め（pointerdown）を数える**こと——
  調整中は指を離さないので click は元々起きず、素通しに戻しても検査が通ってしまう（故障注入ですり抜けた）
- **押し切る前の「あと少し」を連続値で見せる**（`pinchProgress` → `--airtouch-progress`）。
  押した瞬間だけ変わる表示だと、空振りしたとき何が足りないのか利用者には分からない
- **中指つまみ＝右クリックは、人差し指が開いていることを条件に入れる**。
  摘まむと中指も一緒に親指へ寄るので、中指だけを見ると**普通のタップのたびにメニューが出る**
- **スクロールは速さで倍率を変える**（`scrollGain`・最大3.2倍）。等倍のままだと長い記事で腕が
  何往復も要る。ゆっくり動かすときは1:1のままにして狙いを崩さない
- **静止クリック（止めるだけで押す）は既定OFF**。止めているだけで押されるので誤爆が必ず起きる。
  ONにするのは、つまむのが難しい人が自分で選んだときだけ。**横払いで戻る**も同じ理由で既定OFF
  （書きかけの入力が消える）
- **できることの一覧は起動時に一度だけ出す**。何ができるか分からないまま手をかざしても何も起きず、
  「動かない」と判断されて二度と使われない。**切ってある操作は一覧に載せない**——
  書いてあるのに反応しないと、壊れていると受け取られる
- **設定は保存する**（`SETTINGS_KEY`・localStorage）。開くたびに調整させると「使うまでが面倒な機能」になる。
  ただし**知らない鍵は素通ししない／上下が逆の閾値は捨てる**（逆のままだと押下が一度も成立しない）。
  localStorage はシークレットタブ・容量枯渇で `getItem` ごと落ちるので、読み書きは必ず try/catch で既定へ落とす
- **細かい設定は動いている間だけ画面に出す**。止まっているときに並んでいても押して効かず、
  「押したのに反応しない＝壊れている」になる
...
**全エージェント共通の運用規約は `.claude/agent-conventions.md` が唯一の正**（契約書式＝受け取る/返す・
最小権限の `tools:` 指定・コンテキスト節約・**必須検査の対応表**・停止条件/エスカレーション・振り分け表）。
各エージェント定義は「自分の専門」だけを書き、共通事項を重複させない（重複させると片方だけ古くなる）。
規約への適合は `harness-lint.sh` 検査#14 が機械検査する。
...
- **Legal-Checker・i18n・Asset-Guardianと並列で実行**し、Dynamic-Testerの前に完了させる
...
- **Legal-Checker・Security・Asset-Guardianと並列で実行**し、Dynamic-Testerの前に完了させる
...
### Asset-Guardianエージェント (`asset-guardian`)
- 画像・音声アセットが方針どおりか（WebP・解像度不変・アルファ保持・参照整合・容量）を検査する品質ゲート
- **Legal-Checker・Security・i18nと並列で実行**し、Dynamic-Testerの前に完了させる
- 判定は BLOCK / WARN / OK。**BLOCKが1件でも残る場合はDynamic-Testerへ進ませない**
- 検査は `verify-asset-format.mjs` / `verify-game-assets.mjs` / `verify-known-bug-patterns.mjs` の3本＋容量確認。
  **新規アセットは git add 前＝未追跡**なので未追跡ファイルも対象にする
- 絵の良し悪しは見ない（それは Graphic-Designer）。見るのは「方針から外れていないか」「無言で壊れていないか」だけ
- 画像そのものの作り直しは Graphic-Designer、参照コードの修正は Code-Generator へ回す
...
### Triageエージェント (`triage`)
- 「動かない」「おかしい」という症状ベースの報告から**再現手順と観測事実**を作り、壊れている層を1つに絞って担当へ渡す
- **直さない**（Edit/Writeを持たない）。切り分けながら直すと何が効いたのか分からなくなるため
- 出力に**捨てた仮説とその棄却理由**を必ず含める。層の切り分け順はアセット→描画→通信→実行環境→データ→ロジック
- **起動条件**: 深澤からの症状報告 ／ 同じFAILが2回以上戻ってきた ／ 複数の層にまたがって担当が決まらない
- 二度目に踏んだ不具合は Verifier へ「機械検査への昇格」を提案する

### Verifierエージェント (`verifier`)
- 一度踏んだ「例外もエラーも出ない不具合」を `scripts/verify-*.mjs` の機械検査へ昇格させる検査職人
- **故障を仕込んで✗が出ることを確認するまで納品しない**（「緑になった」を成果にしない）。偽の緑は検査が無いより悪い
- 登録は3箇所すべて（`scripts/` の実体 ／ `.claude/agent-conventions.md` §4 の対応表 ／ CLAUDE.md の該当節）。
  1つでも漏れると次のセッションが検査の存在に気づかない
- **起動条件**: 同じ無言バグを二度踏んだ ／ Triageが根本原因を特定した ／ 既存検査に穴が見つかった ／
  Plannerが新しい受け入れ条件を必要としている
...
深澤(PM) → [Planner] 要件定義・設計書作成（市場調査なしの場合／ゲーム企画は specs/ 保存＋スケルトンまで）
          ...
          └→ [Code-Generator]   実装（グラフィック・音楽納品後）
               ↓
     ┌──────────────── 品質ゲート（4体を並列実行）────────────────┐
     │ [Legal-Checker] 法務   [Security] 脆弱性                     │
     │ [i18n] 日英            [Asset-Guardian] アセット方針・参照整合 │
     └────────────────────────────────────────────────────────────┘
               ↓ RED/YELLOW（グラフィック起因）
            ...
            [Code-Generator] 修正 → 該当ゲートへ再チェック
               ↓ BLOCK（アセット方針違反・参照切れ）
            [Asset-Guardian] 指摘 → [Graphic-Designer]（画像） / [Code-Generator]（参照） → 再チェック
               ↓ GREEN / OK / 非BLOCK（4体すべて通過）
     ...
     ┌──── 常設ユーティリティ（どのフェーズからでも起動）────┐
     │ [Triage] 症状→再現→層の切り分け → 担当エージェントへ    │
     │ [Verifier] 二度踏んだ無言バグ → 機械検査へ昇格          │
     └──────────────────────────────────────────────────────┘
